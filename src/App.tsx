...
import { IS_SHIFT_MODULE_ENABLED } from './config';
import { api } from './api';
...
export default function App() {
  ...
  const [currentTab, setCurrentTab] = useState<string>(() => {
    ...
    return hash && ['home', 'shifts', 'inventory', 'sales', 'dispatch', 'billing', 'seller-debts', 'business-debts', 'daily-sales', 'my-sales', 'team', 'clients', 'terms', 'privacy'].includes(hash.split('?')[0]) ? hash.split('?')[0] : 'home';
  });
  ...
  React.useEffect(() => {
    const handleHashChange = () => {
      ...
      if (['home', 'shifts', 'inventory', 'sales', 'dispatch', 'billing', 'seller-debts', 'business-debts', 'daily-sales', 'my-sales', 'team', 'clients', 'terms', 'privacy'].includes(cleanHash)) {
  ...
  React.useEffect(() => {
    ...
    if (ios && !standalone) {
      const dismissed = localStorage.getItem('pos_pwa_ios_dismissed');
      if (!dismissed) {
              ...
              <div className="p-6 relative">
                ...
                <div className="flex items-center gap-3.5 mb-5 mt-2">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center border border-teal-100 shadow-sm shrink-0">
                    <img src="/logo_final.jpg" alt="Sistema POS" className="w-9 h-9 object-contain rounded-lg" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-slate-800 leading-tight">Instalar Sistema POS App</h3>
                    <p className="text-xs text-slate-600 font-bold mt-0.5">Acceso rápido • Soporte Offline</p>
                  </div>
                </div>

                <div className="space-y-4 text-slate-650 text-sm mb-6">
                  <p className="text-slate-500 text-xs">
                    Sigue estas simples instrucciones para añadir el Sistema POS a tu dispositivo como una aplicación nativa.
                  </p>
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95">
              <div className="p-6 relative">
                ...
                <div className="text-center mb-5 mt-2">
                  ...
                  <h3 className="text-lg font-bold text-slate-800">Descargar Aplicación POS</h3>
