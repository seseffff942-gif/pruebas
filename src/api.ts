...
export const api = {
  ...
  uploadProductImage: async (id: string, file: File): Promise<{ success: boolean; image: string; bgRemoved?: boolean }> => {
  ...
  uploadAndRemoveBackground: async (file: File): Promise<{ success: boolean; imageUrl: string; bgRemoved?: boolean }> => {
    const compressedFile = await compressImage(file, 1024, 1024, 0.85);
    const formData = new FormData();
    formData.append('image', compressedFile);
    const res = await fetchWithAuth('/api/upload-image/remove-bg', {
      method: 'POST',
      body: formData
    });
    const resData = await safeJson(res);
    if (!res.ok) {
      throw new Error(resData.error || 'Error procesando la imagen con Cloudinary');
    }
    return resData;
  },
  ...
  testWhatsApp: async (phone: string) => {
    const res = await fetchWithAuth('/api/whatsapp/send', {
      ...
      body: JSON.stringify({
        phone,
        message: "¡Hola! Este es un mensaje de prueba desde el Sistema POS. Su configuración de WhatsApp funciona correctamente. 🚀"
      })
    });
    if (!res.ok) {
