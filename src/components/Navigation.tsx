...
import { IS_SHIFT_MODULE_ENABLED, APP_NAME } from '../config';
...
export function Navigation({ user, activeUser, currentTab, onChangeTab, onLogout, onReturnToAdmin, onImpersonate, isMobile, onShowInstallGuide, showInstallButton }: NavigationProps) {
  ...
  const [currentTheme, setCurrentTheme] = useState(() => {
    return localStorage.getItem('agricovet_theme_palette') || 'emerald';
  });
  const [showThemeModal, setShowThemeModal] = useState(false);

  useEffect(() => {
    document.documentElement.setAttribute('data-theme', currentTheme);
    localStorage.setItem('agricovet_theme_palette', currentTheme);
  }, [currentTheme]);

  const THEMES = [
    {
      id: 'emerald',
      name: 'Verde Esmeralda (Agro)',
      desc: 'Tonalidad campestre clásica y profesional',
      badge: 'bg-emerald-600',
      gradient: 'from-[#0c5c35] to-[#042616]',
      ring: 'ring-emerald-500',
    },
    {
      id: 'blue',
      name: 'Azul Corporativo',
      desc: 'Elegancia ejecutiva e industrial',
      badge: 'bg-blue-600',
    ...
    },
    {
      id: 'teal',
      name: 'Teal Moderno',
      desc: 'Tono cibernético y tecnológico fresco',
      badge: 'bg-teal-600',
    ...
    },
    {
      id: 'amber',
      name: 'Ámbar Cosecha',
      desc: 'Cálido, dorado y de alta visibilidad',
      badge: 'bg-amber-600',
    ...
    {
      id: 'dark',
      name: 'Modo Oscuro Onyx',
      desc: 'Fondo titanio de alto contraste nocturno',
      badge: 'bg-slate-800',
  ...
  const navItems = [
    ...
    ...(IS_SHIFT_MODULE_ENABLED ? [{ id: 'shifts', label: 'Turnos / Caja', icon: Clock, roles: ['admin', 'seller'] }] : []),
      ...
      <header className="md:hidden sticky top-0 z-50 bg-white border-b border-slate-200 flex justify-between items-center px-4 h-16 w-full select-none">
        <div className="flex items-center gap-1.5">
          ...
          <button 
            ...
            className="text-slate-500 hover:text-slate-700 relative p-1.5 hover:bg-slate-50 rounded-lg transition-transform active:scale-95 cursor-pointer" 
          ...
          <div className="md:hidden">
            ...
            >
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-3 rounded-2xl bg-emerald-50/60 border border-emerald-100 text-slate-800 transition-all active:scale-[0.98] cursor-pointer"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="text-[12px] font-extrabold text-slate-800 leading-none">Paleta de Colores</p>
                    <p className="text-[10px] text-emerald-700 font-bold mt-1">Tema: {THEMES.find(t => t.id === currentTheme)?.name}</p>
                  </div>
                </div>
                <span className="text-[10px] font-black uppercase text-emerald-700 bg-emerald-100 px-2.5 py-1 rounded-lg">Cambiar</span>
              </button>
      ...
      <header className="hidden md:flex fixed top-0 left-[260px] right-0 h-16 bg-white border-b border-[#e1e3e4] items-center justify-between px-10 z-40">
        ...
        <div className="flex items-center gap-6 ml-8 relative">
          <div className="flex items-center gap-2 relative">
            <button 
              onClick={() => setShowThemeModal(true)} 
              className="p-2 text-[#44474c] hover:text-[#00696a] transition-all relative flex items-center justify-center rounded-xl hover:bg-slate-50 cursor-pointer active:scale-95" 
              title="Cambiar paleta de colores del sistema"
            >
              <Palette size={20} />
            </button>
            ...
            <div className="p-6 pb-3 border-b border-slate-100 relative shrink-0">
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-400 mt-1 font-bold tracking-wide uppercase">
                  Enterprise POS & Inventory Engine
                </p>
              </div>
            </div>
            ...
            >
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-extrabold text-lg text-slate-800 leading-tight">Paleta de Colores</h3>
                    <p className="text-xs text-slate-500 font-medium">Personaliza la apariencia visual de la plataforma</p>
                  </div>
                </div>
                <button 
                ...
                {THEMES.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex flex-col text-left p-4 rounded-2xl border transition-all cursor-pointer relative overflow-hidden group",
                        isSelected 
                          ? "bg-slate-50 border-emerald-500 ring-2 ring-emerald-500/30 shadow-md" 
                          : "bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50/80"
                      )}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={cn("w-4 h-4 rounded-full shadow-xs", t.badge)} />
                          <span className="text-xs font-bold text-slate-900">{t.name}</span>
                        </div>
                        {isSelected && (
                          <span className="w-5 h-5 bg-emerald-500 text-white rounded-full flex items-center justify-center text-[10px] font-black shadow-xs">
                            ✓
                          </span>
                        )}
                      </div>

                      <p className="text-[11px] text-slate-500 line-clamp-2 font-medium mb-3">
                        {t.desc}
                      </p>

                      {/* Visual Color Bar Swatch Preview */}
                      <div className="h-3 w-full rounded-lg overflow-hidden flex border border-black/5">
                        <div className={cn("flex-1 bg-gradient-to-r", t.gradient)} />
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <span className="text-xs text-slate-400 font-semibold">Se aplica al instante</span>
                <button
                  onClick={() => setShowThemeModal(false)}
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-sm shadow-emerald-600/20 transition-all cursor-pointer"
                >
                  Guardar Preferencia
                </button>
              </div>
