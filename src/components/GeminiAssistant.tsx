...
export function GeminiLogo({ size = 24, animate = true, className }: { size?: number; animate?: boolean; className?: string }) {
  return (
    <img 
      src="/logo_final.jpg" 
      alt="POS Assistant" 
      ...
      className={className || `object-contain rounded-full shadow-sm ${animate ? "animate-pulse" : ""}`}
    />
  );
}
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      role: 'model',
      content: '¡Hola! Soy el asistente inteligente del Sistema POS. ¿En qué te puedo ayudar hoy? (Consultar disponibilidad de productos, redactar recordatorios por WhatsApp, etc.)'
    }
  ]);
