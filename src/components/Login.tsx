...
import { Lock, Mail, User as UserIcon, Monitor, Smartphone, Sparkles, AlertTriangle, CheckCircle, ArrowRight, RefreshCw, Star, ShieldCheck, Check } from 'lucide-react';
import { api } from '../api';
...
export function Login({ onLogin }: LoginProps) {
  ...
  const handleRegisterIntent = async (e: React.FormEvent) => {
    ...
    if (!passValidation.isValid) {
      return setError(passValidation.errorMessage || 'La contraseña debe tener al menos 8 caracteres, una mayúscula, una minúscula, un número y un carácter especial.');
    }
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (!passValidation.isValid) {
      return setError(passValidation.errorMessage || 'La contraseña no cumple con los requisitos de seguridad.');
    }
    ...
    <div className="min-h-screen bg-[#050c09] text-slate-100 flex font-sans relative overflow-hidden select-none">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-[#030906] flex-col justify-between p-12 overflow-hidden border-r border-white/5">
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            <span translate="no" className="notranslate font-bold text-xs uppercase tracking-[0.25em] text-emerald-400 font-manrope">POS Enterprise</span>
            <p className="text-[10px] text-slate-400">Sistema POS & Control de Inventario</p>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-center justify-between text-xs text-slate-500 border-t border-white/[0.05] pt-6">
          <p>© 2026 <span translate="no" className="notranslate">POS Enterprise</span>. Todos los derechos reservados.</p>
      ...
      <div className="flex-1 flex flex-col justify-center items-center p-4 sm:p-8 lg:p-12 z-10 min-h-screen">
        <div className="w-full max-w-[440px]">
              ...
              {mode === 'register' && (
                <form onSubmit={handleRegisterIntent} className="space-y-5">
                  <div className="space-y-4">
                    ...
                    {/* Password choice */}
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Crear Contraseña Segura</label>
                      <div className="relative group/input">
                        ...
                        <input
                          ...
                          value={password}
                          onChange={(e) => setPassword(e.target.value)}
                          placeholder="Mín. 8 caract., Mayús, Minús, Num, Especial"
                          className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-white/10 focus:border-emerald-500/50 outline-none transition-all bg-white/[0.02] hover:bg-white/[0.04] text-white placeholder-slate-500 text-sm font-semibold focus:ring-4 focus:ring-emerald-500/5"
                          required
                        />
                      </div>

                      {/* Real-time Password Security Validation Indicator */}
                      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 space-y-1.5 mt-2">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Requisitos de Contraseña:</p>
                        <div className="grid grid-cols-2 gap-1.5 text-[11px] font-medium">
                          <div className={`flex items-center gap-1.5 ${passValidation.hasMinLength ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>
                            {passValidation.hasMinLength ? <Check size={12} className="text-emerald-400 shrink-0" /> : <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 ml-1" />}
                            <span>Min. 8 caracteres</span>
                          </div>
                          <div className={`flex items-center gap-1.5 ${passValidation.hasUppercase ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>
                            {passValidation.hasUppercase ? <Check size={12} className="text-emerald-400 shrink-0" /> : <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 ml-1" />}
                            <span>1 Mayúscula (A-Z)</span>
                          </div>
                          <div className={`flex items-center gap-1.5 ${passValidation.hasLowercase ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>
                            {passValidation.hasLowercase ? <Check size={12} className="text-emerald-400 shrink-0" /> : <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 ml-1" />}
                            <span>1 Minúscula (a-z)</span>
                          </div>
                          <div className={`flex items-center gap-1.5 ${passValidation.hasNumber ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>
                            {passValidation.hasNumber ? <Check size={12} className="text-emerald-400 shrink-0" /> : <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 ml-1" />}
                            <span>1 Número (0-9)</span>
                          </div>
                          <div className={`flex items-center gap-1.5 col-span-2 ${passValidation.hasSpecialChar ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>
                            {passValidation.hasSpecialChar ? <Check size={12} className="text-emerald-400 shrink-0" /> : <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 ml-1" />}
          ...
          <div className="text-center mt-6 space-y-1 text-slate-500 text-xs select-none pointer-events-none">
            <p>POS Enterprise Security System</p>
