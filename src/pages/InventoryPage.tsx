...
import { cn, doesNotNeedStock, isCriticalStock, downloadProductsPdf } from '../utils';
        ...
        <div className="bg-white p-4 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4 sm:space-y-6">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <div className={cn("flex w-full lg:w-auto items-center justify-end gap-2.5", inventoryViewMode === 'office' ? 'hidden' : '')}>
              <button
                onClick={() => {
                  const headers = ["ID", "Producto", "Categoría", "Stock Físico", "Precio Unitario (Q)", "Valor Total (Q)"];
                  let csvLines: string[] = [headers.join(",")];
                  let totalValuation = 0;

                  filteredProducts.forEach(p => {
                    if (p.is_external) return;
                    const cleanName = (p.name || 'Sin nombre').replace(/"/g, '""');
                    const cleanCat = (p.category || 'General').replace(/"/g, '""');
                    const stock = Number(p.stock) || 0;
                    const price = Number(p.price) || 0;
                    const valuation = stock * price;
                    totalValuation += valuation;

                    csvLines.push(`"${p.id || ''}","${cleanName}","${cleanCat}",${stock},${price.toFixed(2)},${valuation.toFixed(2)}`);
                  });

                  csvLines.push(`"","VALORACIÓN TOTAL DE INVENTARIO","","",0,${totalValuation.toFixed(2)}`);
                ...
                title="Descargar Inventario en CSV optimizado"
              >
                <Download size={14} />
              ...
              <button
                onClick={() => downloadProductsPdf(filteredProducts)}
                className="flex items-center justify-center gap-1.5 h-11 px-4 border border-slate-300 text-slate-800 hover:bg-slate-100 rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer bg-white shrink-0"
              ...
              <button
                  ...
                  try {
                    ...
                    filteredProducts.forEach(p => {
                      ...
                      rows.push({
                        ...
                        'Categoría': category,
                        'Stock Físico': stock,
                        'Precio Unitario (Q)': price,
                        'Valor Total (Q)': valuation
                      });
                    });
                    
                    rows.push({
                      'ID': '',
                      'Producto': 'VALORACIÓN TOTAL:',
                      'Categoría': '',
                      'Stock Físico': '',
                      'Precio Unitario (Q)': '',
                      'Valor Total (Q)': totalValuation
                    });
                ...
                title="Descargar Inventario (Excel)"
              >
                <Download size={14} />
              ...
              {user.role === 'admin' && (
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 h-11 px-6 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black tracking-wider transition-all shadow-md hover:shadow-lg active:scale-95 whitespace-nowrap cursor-pointer"
                >
                  <Plus size={16} />
