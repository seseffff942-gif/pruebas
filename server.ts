...
const JWT_SECRET = process.env.JWT_SECRET || "default_stable_secret_for_pos_system_dev";
...
/**
 * Processes an image using Cloudinary API to automatically remove the background.
 * First uploads to Cloudinary with background removal effect/addon, fetches the resulting transparent PNG,
 * and returns the transparent image buffer to be saved into Supabase.
 */
async function processImageWithCloudinaryBackgroundRemoval(
  imageBuffer: Buffer,
  publicIdPrefix: string = "product"
): Promise<{ buffer: Buffer; contentType: string; cloudinaryUrl?: string; bgRemoved: boolean }> {
  const isCloudinaryConfigured = Boolean(
    process.env.CLOUDINARY_URL ||
    (process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_CLOUD_NAME !== 'demo' && process.env.CLOUDINARY_API_KEY)
  );

  if (!isCloudinaryConfigured) {
    console.warn("[Cloudinary] Credentials not configured in .env. Using transparent PNG conversion fallback.");
    try {
      const transparentBuffer = await sharp(imageBuffer).png().toBuffer();
      return { buffer: transparentBuffer, contentType: 'image/png', bgRemoved: false };
    } catch (e) {
      return { buffer: imageBuffer, contentType: 'image/png', bgRemoved: false };
    }
  }

  try {
    console.log(`[Cloudinary] Uploading image (${imageBuffer.length} bytes) for background removal processing...`);

    // 1. Upload stream to Cloudinary with background removal requested
    const uploadResult = await new Promise<any>((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: "agricovet_products",
          public_id: `${publicIdPrefix}_${Date.now()}`,
          background_removal: "cloudinary_ai", // Cloudinary AI background removal addon
          transformation: [
            { effect: "background_removal" }, // Background removal transformation
            { format: "png" }
          ],
          overwrite: true
        },
    ...
    // 2. Build URL with explicit e_background_removal transformation
    let processedUrl = uploadResult?.secure_url;
    if (uploadResult?.public_id) {
      processedUrl = cloudinary.url(uploadResult.public_id, {
    ...
    // 3. Fetch the processed transparent PNG back from Cloudinary
    console.log("[Cloudinary] Fetching background-removed PNG from URL:", processedUrl);
    const fetchResponse = await fetch(processedUrl);

    if (fetchResponse.ok) {
      const arrayBuffer = await fetchResponse.arrayBuffer();
      const transparentPngBuffer = Buffer.from(arrayBuffer);
      console.log(`[Cloudinary] Successfully retrieved transparent image (${transparentPngBuffer.length} bytes).`);
      return {
      ...
      console.warn(`[Cloudinary] Fetching transparent PNG returned HTTP ${fetchResponse.status}. Using secure_url fallback.`);
      return {
        buffer: imageBuffer,
        contentType: 'image/png',
        cloudinaryUrl: uploadResult?.secure_url || processedUrl,
        bgRemoved: false
      };
    }
  } catch (err: any) {
    console.error("[Cloudinary] Background removal failed, using fallback:", err?.message || err);
    try {
      const fallbackBuffer = await sharp(imageBuffer).png().toBuffer();
      return { buffer: fallbackBuffer, contentType: 'image/png', bgRemoved: false };
    } catch {
      return { buffer: imageBuffer, contentType: 'image/png', bgRemoved: false };
    }
  }
 ...
 */
function validatePassword(password: string): { isValid: boolean; errorMessage?: string } {
  const p = password || '';
  ...
  return {
    isValid,
    errorMessage: isValid ? undefined : `La contraseña debe contener: ${errors.join(', ')}.`
  };
}
...
const initialDb = {
  users: [
    ...
    { id: "u1", name: "Admin General", email: "admin2@pos-system.local", role: "admin", photo: "https://i.pravatar.cc/150?u=u1", password: "123" },
    { id: "u1c", name: "Admin 3", email: "admin3@pos-system.local", role: "admin", photo: "https://i.pravatar.cc/150?u=3", password: "123" },
    { id: "u2", name: "Ventas Principal", email: "ventas1@pos-system.local", role: "seller", photo: "https://i.pravatar.cc/150?u=u2", password: "123" },
    { id: "u2b", name: "Ventas 2", email: "ventas2@pos-system.local", role: "seller", photo: "https://i.pravatar.cc/150?u=5", password: "123" },
...
if (!process.env.VERCEL) {
  ...
  async function getFolioMap() {
    ...
    try {
      ...
      console.warn("[FolioMap] Supabase query skipped/offline:", e?.message || e);
    }
    ...
    if (emailUser && emailPass) {
        try {
            ...
            await transporter.sendMail({
                from: `"Sistema POS" <${emailUser}>`,
                to: email,
                subject: "Código de Seguridad - Sistema POS",
                text: `Tu código de acceso/verificación es: ${code}`,
                html: `
                    <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px;">
                        <h2 style="color: #0f172a;">Código de Verificación</h2>
                        <p>Hola,</p>
                        <p>Has solicitado un código para acceder al <b>Sistema POS</b>.</p>
                        ...
                        <p style="font-size: 12px; color: #9ca3af; text-align: center;">© 2026 Sistema POS. Todos los derechos reservados.</p>
                    </div>
                `
            });
  ...
  app.post("/api/auth/register", asyncHandler(async (req: any, res: any) => {
    ...
    if (!passCheck.isValid) {
      return res.status(400).json({ error: passCheck.errorMessage });
    }
  ...
  app.get("/api/users", requireAuth, asyncHandler(async (req: any, res: any) => {
    let users: any[] = [];
    try {
      const { data, error } = await supabase.from("users").select("id, name, email, role, photo, phone");
  ...
  app.get("/api/office-inventory", requireAuth, asyncHandler(async (req: any, res: any) => {
    let data: any[] = [];
    try {
      const resData = await supabase.from("office_inventory").select("*");
      if (resData.data) data = resData.data;
    } catch (e) {}
  ...
  app.get("/api/products", requireAuth, asyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      console.warn("[Products] Supabase unavailable, serving initialDb products:", err?.message || err);
    }

    setCachedData("products", initialDb.products);
    res.json(initialDb.products);
  }));
  ...
  app.post("/api/products", requireAuth, requireAdmin, asyncHandler(async (req: any, res: any) => {
    ...
    // Keep initialDb in sync
    initialDb.products.unshift(product);

    try {
      await supabase.from("products").insert([product]);
    } catch (err: any) {
      console.warn("Supabase insert product warning:", err?.message || err);
    }
    res.json(product);
  }));
    ...
    try {
      ...
      if (existing) {
        await supabase.from("users").update({
          ...
          email: "system-logo@pos-system.local",
          role: "system"
  ...
  app.delete("/api/products/all", requireAuth, requireAdmin, asyncHandler(async (req: any, res: any) => {
    invalidateCache("products");
    initialDb.products = [];
    try {
  ...
  app.delete("/api/products", requireAuth, requireAdmin, asyncHandler(async (req: any, res: any) => {
    invalidateCache("products");
    initialDb.products = [];
    try {
      await supabase.from("products").delete().neq("id", "none");
  ...
  app.delete("/api/products/:id", requireAuth, requireAdmin, asyncHandler(async (req: any, res: any) => {
    invalidateCache("products");
    const { id } = req.params;
    initialDb.products = initialDb.products.filter(p => p.id !== id);
    try {
  ...
  app.post("/api/products/:id/image", requireAuth, requireAdmin, upload.single("image"), asyncHandler(async (req: any, res: any) => {
    ...
    try {
      // 1. First process image with Cloudinary API to remove background automatically
      console.log(`[Cloudinary BG Removal] Processing image for product ID: ${id}...`);
      const processed = await processImageWithCloudinaryBackgroundRemoval(req.file.buffer, `product_${id}`);

      // Ensure bucket exists in Supabase Storage (best effort)
      try {
        const { data: buckets } = await supabase.storage.listBuckets();
        if (buckets && !buckets.find(b => b.name === 'productos')) {
          await supabase.storage.createBucket('productos', { public: true });
      ...
      // 2. Save the processed background-removed PNG image into Supabase Storage
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('productos')
      ...
      let imageUrl = '';
      if (uploadError) {
        console.error("Supabase Storage upload error, failing back to Cloudinary URL or base64:", uploadError);
        if (processed.cloudinaryUrl) {
          imageUrl = processed.cloudinaryUrl;
        } else {
          imageUrl = `data:image/png;base64,${processed.buffer.toString('base64')}`;
        }
      } else {
        const { data: publicUrlData } = supabase.storage
          .from('productos')
      ...
      // Update initialDb in memory
      const prodInDb = initialDb.products.find(p => p.id === id);
      if (prodInDb) {
        prodInDb.image = imageUrl;
      }

      // 3. Update Product in Supabase DB
      ...
      res.json({ 
        ...
        cloudinaryUrl: processed.cloudinaryUrl 
      });
    } catch (error: any) {
      console.error("Image processing error:", error);
      res.status(500).json({ error: "Error procesando la imagen con Cloudinary", details: error.message });
    }
  }));

  // Standalone endpoint to remove background from any image via Cloudinary and store in Supabase
  app.post("/api/upload-image/remove-bg", requireAuth, upload.single("image"), asyncHandler(async (req: any, res: any) => {
    if (!req.file) return res.status(400).json({ error: "No image file provided" });

    try {
      const processed = await processImageWithCloudinaryBackgroundRemoval(req.file.buffer, "custom_upload");
      ...
      res.status(500).json({ error: "Error procesando imagen", details: error.message });
    }
  }));
    ...
    try {
      ...
      try {
        ...
        if (products && products.length > 0) {
          productsContext = `Inventario actual del sistema:\n` + 
            products.map((p: any) => `- ${p.name} (${p.category}): Q${parseFloat(p.price || 0).toFixed(2)}`).join("\n") + "\n\n";
        }
      } catch (dbErr) {
        productsContext = "El sistema contiene productos y artículos del catálogo comercial.\n\n";
      }

      const systemInstruction = `Eres el "Asistente Inteligente de Sistema POS Enterprise", una IA integrada en el sistema de gestión de inventario y punto de venta.
Puedes ayudar a los vendedores y administradores con las siguientes tareas:
1. Recomendar productos del inventario y responder dudas técnicas o comerciales.
...
4. Brindar pautas generales rápidas sobre atención comercial y catálogo de productos.
